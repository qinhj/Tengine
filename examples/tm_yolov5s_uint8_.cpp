...
// load input images
static void get_input_data_focus(const char *image_file, uint8_t *input_data, float input_scale, int zero_point) {
            ...
            for (int c = 0; c < 3; c++) {
                for (int h = 0; h < lb.h / 2; h++) {
                    for (int w = 0; w < lb.w / 2; w++) {
                        ...
                        /* quant to uint8 */
                        int udata = (int)round(input_temp[in_index] / input_scale + (float)zero_point);
