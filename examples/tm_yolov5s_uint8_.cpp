...
// load input images
static void get_input_data_focus(const char *image_file, image &lb, float input_scale, int zero_point) {
    ...
    imi_utils_yolov5_focus_data(input_temp, lb, input_scale, zero_point);
    free(input_temp);
}
...
static void show_usage(const char *exe) {
    ...
    fprintf(stdout, "   %s -m yolov5s.v5.uint8.tmfile -i imilab_640x360x3_bgr_catdog.rgb24 -o output/imilab_640x360x3_bgr_catdog.rgb24 -t 4 -f 200\n", exe);
    fprintf(stdout, "   # specific class of coco 80 classes(e.g. person)\n");
    fprintf(stdout, "   %s -m yolov5s.v5.uint8.tmfile -i imilab_640x360x3_bgr_human1.rgb24 -o output/imilab_640x360x3_bgr_human1.rgb24 -t 4 -f 100 -c 0\n", exe);
    fprintf(stdout, "   %s -m yolov5s.v5.uint8.tmfile -i imilab_640x360x3_bgr_human2.rgb24 -o output/imilab_640x360x3_bgr_human2.rgb24 -t 4 -f 500 -c 0\n", exe);
    fprintf(stdout, "   # single class(e.g. person)\n");
    fprintf(stdout, "   %s -m yolov5s.uint8.tmfile -i imilab_640x360x3_bgr_catdog.rgb24 -o output/imilab_640x360x3_bgr_catdog.rgb24 -t 4 -f 200 -n 1\n", exe);
    fprintf(stdout, "   %s -m yolov5s.uint8.tmfile -i imilab_640x360x3_bgr_human1.rgb24 -o output/imilab_640x360x3_bgr_human1.rgb24 -t 4 -f 100 -n 1\n", exe);
    fprintf(stdout, "   %s -m yolov5s.uint8.tmfile -i imilab_640x360x3_bgr_human2.rgb24 -o output/imilab_640x360x3_bgr_human2.rgb24 -t 4 -f 500 -n 1\n", exe);
}
...
#ifdef USE_OPENCV
    get_input_data_focus(image_file, lb, input_scale, input_zero_point);
