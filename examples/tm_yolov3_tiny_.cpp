...
// yolov3
const float *biases = anchors;
// yolov3-tiny
const float *biases_tiny = anchors_tiny;
// yolov2
float biases_yolov2[10] = {
    0.57273, 0.677385,
    1.87446, 2.06253,
    3.33843, 5.47434,
    7.88282, 3.52778,
...
void get_input_data_darknet(const char *image_file, image &lb) {
    image im = load_image_stb(image_file, 3);
    for (int i = 0; i < im.c * im.h * im.w; i++) {
        im.data[i] = im.data[i] / 255;
    }

    int size = 3 * lb.w * lb.h;
    image sized = letterbox(im, lb.w, lb.h);
    memcpy(lb.data, sized.data, size * sizeof(float));
...
int main(int argc, char* argv[]) {
    ...
    image &lb = yolov3_tiny.lb;

    const int classes = coco_class_num;
    ...
    //imi_utils_tm_show_graph(graph, 0);

    /* set the input shape to initial the graph, and prerun graph to infer shape */
    int dims[] = { 1, 3, lb.h, lb.w }; // nchw
    ...
    if (NULL == lb.data) lb.data = (float *)calloc(sizeof(float), img_size);
    ...
    tm_tensor_output_s tensor[NODE_CNT_YOLOV3_TINY];
    if (imi_utils_tm_get_graph_tensor(graph, tensor, NODE_CNT_YOLOV3_TINY, 0) < 0) {
        fprintf(stderr, "[%s] get output tensor info failed\n", __FUNCTION__);
        return -1;
    }

    /* prepare process input data, set the data mem to input tensor */
    get_input_data_darknet(image_file, lb);
    ...
    for (int i = 0; i < output_node_num; ++i) {
        ...
        //_imi_utils_tm_show_tensor(out_tensor, i, 0, 0);
        layer l_params;
        int out_w = 0 == i ? 26 : 13;
        int out_h = 0 == i ? 26 : 13;
        l_params = make_darknet_layer(1, out_w, out_h, lb.w, lb.h, numBBoxes, total_numAnchors, classes, layer_type);
    ...
    // get network boxes
    detection* dets =
        get_network_boxes(layers_params, img.w, img.h, lb.w, lb.h, thresh, hier_thresh, 0, relative, &nboxes);
    ...
    free(dets);
    save_image(img, "tengine_yolov3_tiny_out");
