    ...
    for (h = 0; h < im.h; h++)
    {
        for (w = 0; w < im.w; w++)
        {
            for (c = 0; c < channels; c++)
            {
                ...
                {
                    if (bgr)
                        idx = w + im.w * h + (im.c - 1 - c) * im.w * im.h;
                    else
                        idx = w + im.w * h + c * im.w * im.h;
                    //printf("%d %d %d: %d\n", w, h, c, idx);
    ...
    FILE* fp, const image img, char bgr, const image lb, const float cov[][3])
{
    ...
    if ((img.w <= img.h && lb.h != img.h) || (img.h <= img.w && lb.w != img.w))
    {
    ...
    for (h = 0; h < img.h; h++)
    {
        for (w = 0; w < img.w; w++)
        {
            for (c = 0; c < img.c; c++)
            {
                ...
                {
                    if (bgr)
                        idx = w + img.w * h + (img.c - 1 - c) * img.w * img.h;
                    else
                        idx = w + img.w * h + c * img.w * img.h;
                    //printf("w=%d h=%d c=%d idx=%d\n", w, h, c, idx);
                    img.data[idx] = (float)uc;
                    if (bgr)
                        idx_ = (w + dw) + lb.w * (h + dh) + (img.c - 1 - c) * lb.w * lb.h;
                    else
                        idx_ = (w + dw) + lb.w * (h + dh) + c * lb.w * lb.h;
