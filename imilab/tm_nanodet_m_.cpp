...
#include "utils/imi_model_nanodet.hpp"

// postprocess threshold
static float prob_threshold = 0.6f; // 0.4f;
static float nms_threshold = 0.40f; // 0.50f;

// example models for show usage
static const char *models[] = {
    "nanodet_m.tmfile",     // official model
    "nanodet_m.imi.tmfile", // imilab model
};

// time cost
static double start_time = 0.;

// @brief:  nanodet output tensor postprocess
// @param:  model[in]   input nanodet model info
// @param:  graph[in]   input nanodet graph inst
// @param:  buffer[in]  output tensor buffer
// @param:  proposals   output detected boxes
static int proposals_objects_get(const nanodet &model,
    graph_t &graph, const void *buffer[], std::vector<Object> &proposals) {
    proposals.clear();

    /* generate output proposals */
    return imi_utils_nanodet_proposals_generate(model, buffer, prob_threshold, proposals);
}
...
int main(int argc, char* argv[]) {
    ...
    nanodet &model = nanodet_m;
    image input = make_empty_image(320, 180, 3);
    ...
    /* Note: nanodet isn't support to change input tensor shape within
    ...
    */
    ...
    const void *buffer[2 * BR_CNT_NANODET] = { 0 };
    for (int idx = 0; idx < model.outputs; idx++) {
        tensor_t cls_tensor = get_graph_tensor(graph, model.cls_pred_name[idx]);
        tensor_t dis_tensor = get_graph_tensor(graph, model.dis_pred_name[idx]);
        if (NULL == cls_tensor || NULL == dis_tensor) {
            log_error("get graph tensor failed\n");
            return -1;
        }
        buffer[2 * idx + 0] = (const float *)get_tensor_buffer(cls_tensor);
        buffer[2 * idx + 1] = (const float *)get_tensor_buffer(dis_tensor);
    }
    ...
    if (strstr(image_file, "bgra")) input.c = 4, bgr = 0;
    else if (strstr(image_file, "bgr")) input.c = 3, bgr = 0;
    else if (strstr(image_file, "rgba")) input.c = 4, bgr = 1;
    else if (strstr(image_file, "rgb")) input.c = 3, bgr = 1;
    else { ; }
    if (-1 != bgr) {
